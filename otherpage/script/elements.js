...
export class Pion2{
    ...
    constructor(indicePion,Case){
        this.case_courante = Case;
        this.indicePion = indicePion;
        this.caseActu = 0;
        this.oy=88;
        this.ox = ((indicePion==1)?(2):((indicePion==2)?(5):(indicePion==3)?8:11));
        $("#pion"+this.indicePion).attr("style","top:"+parseInt(this.oy)+"%");
        $("#pion"+this.indicePion).css("left",parseInt(this.ox)+"%");
    }
    deplacer(nbrCase,memeCase=0){
        var ox=0;
        var posActuXY =[
            [0,0], 
            [2,88], //joueur 1  
        ...
        var aty = setInterval(()=>{
            //this.caseActu+=1;
            ((this.caseActu+nbrCase)>37)?(this.caseActu+=(37-this.caseActu)):(this.caseActu+= 1);
            switch (memeCase){
                case 1: {
                    ox = pos[this.caseActu].x+1;
                    break;
                }
                case 2:{
                    ox = pos[this.caseActu].x-1;
                ...
                case 3: ox = pos[this.caseActu].x+2;
                    break;
                case 4: ox = pos[this.caseActu].x-2;
                    break;
                default:
                    ox = pos[this.caseActu].x;
            }
            posActuXY[this.indicePion][0]=(this.caseActu===0)?posActuXY[this.indicePion][0]:(ox);
            posActuXY[this.indicePion][1]=(this.caseActu===0)?posActuXY[this.indicePion][1]:(pos[this.caseActu].y);
            $("#pion"+this.indicePion).attr("style","top:"+parseInt(posActuXY[this.indicePion][1])+"%");
            $("#pion"+this.indicePion).css("left",parseInt(posActuXY[this.indicePion][0])+"%"); //mouvement sur x
            k++;
            if (k>nbrCase-1){
                clearInterval(aty);
                return 0;
